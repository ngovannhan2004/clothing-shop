    ...
    <form action="{{ route('admin.categories.update', $editCategory->id) }}" method="post"
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input value="{{$editCategory->name}}" type="text" class="form-control" name="name"
                    ...
                    @foreach ($categories as $category)
                        @if(!$category->id != $editCategory->id)
                            @if ($category->parent_id == 0)
                                <option @if($category->id == $editCategory->parent_id) selected
                                        @endif value="{{ $category->id }}">{{ $category->name }}</option>
                                @foreach ($category->childrens as $childCategory)
                                    @include('admin.partials.option_category_edit', ['childCategory' => $childCategory, 'name_extra' => '--'])
                                @endforeach
                            @endif
                        @endif
                    @endforeach
